class AdminsController < ApplicationController
  ...
  def list_found_products
    @products = AliexpressScraper.products_in_category(params[:keyword], params[:category][:fields][1], params)
    Product.new_product(@products, params[:category][:fields][1])
  end

  def save_approved_products
    products = Product.where(productId: "ANY(array[#{params[:productId]}])")
